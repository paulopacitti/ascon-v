...
forceinline void ascon_decrypt_debug(ascon_state_t *s, uint8_t *m,
...
int crypto_aead_encrypt_opt64(unsigned char *c, unsigned long long *clen,
...
int crypto_aead_decrypt_opt64(unsigned char *m, unsigned long long *mlen,
...
int crypto_aead_decrypt_opt64_debug(
    ...
    *mlen = clen = clen - CRYPTO_ABYTES;
    ...
    ascon_initaead(&s, &key, npub);
    ascon_adata(&s, ad, adlen);
    ascon_decrypt_debug(&s, m, c, clen, timer);
    ascon_final(&s, &key);
    /* verify tag (should be constant time, check compiler output) */
    s.x[3] ^= LOADBYTES(c + clen, 8);
    s.x[4] ^= LOADBYTES(c + clen + 8, 8);
